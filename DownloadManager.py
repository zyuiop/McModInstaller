...
class DownloadManager:
    ...
    def download(self, pkg):
        ...
        if pkg.get("modtype") == None: # C'est un client
            ...
            if os.path.isdir(vanilladir):
                if os.path.exists(vanilladir + "/" + pkg['version'] + ".jar"):
                    ...
                    if "jarfile" in pkg.keys():
                        ...
                        if !self.parent.remote.downloadFile(self.parent.remote, 'http://' + self.parent.remote.repo + self.parent.remote.directory + pkg['jarfile'], profilepath + "/" + pkg['profilename'] + '.jar', "Téléchargé : {} / {}", "Téléchargement du minecraft.jar"):
                    ...
                    if !self.parent.remote.downloadFile(self.parent.remote, 'http://' + self.parent.remote.repo + self.parent.remote.directory + pkg['json'], profilepath + "/" + pkg['profilename'] + ".json"):                        self.appendConsole("Échec lors du téléchargement du profil")
            else:
            ...
            i = 0
            worked = false
            while !worked and i < len(pkg['mirrors']):
